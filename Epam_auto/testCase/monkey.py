...
from epamBLL import appCase as b_app_case
from epamMode import appCase as m_app_case
from testRunner.runnerBase import TestInterfaceCase as te
from epamBLL import apkBase
...
class testMonkey(te):
    ...
    def setUp(self):
        self.l_devices["apkpath"]="../img/monkneyTest.apk"
        super(testMonkey, self).setUp()
        self.bc = b_app_case.GetAppCase(test_module="crash test", GetAppCaseInfo=m_app_case.GetAppCaseInfo, GetAppCase=m_app_case.GetAppCase, fps=[], cpu=[], men=[],
                                        driver=self.driver, package=self.get_apk_pkg(), devices=self.l_devices["deviceName"])
    def monkey_crash(self):
        _yaml = ""
        if self.l_devices["deviceName"] == "emulator-5554":
